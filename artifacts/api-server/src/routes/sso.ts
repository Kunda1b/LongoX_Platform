...
router.post("/auth/sso/configure", async (req: Request, res: Response): Promise<void> => {
  ...
  if (existing) {
    await db.update(ssoConnectionsTable).set({
...
async function exchangeCodeForUserInfo(
  ...
  try {
    ...
    if (provider === "google") return { id: userData.id, email: userData.email, name: userData.name };
    if (provider === "github") return { id: String(userData.id), email: userData.email ?? userData.login, name: userData.name ?? userData.login };
