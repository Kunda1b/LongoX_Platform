...
type TemplateType = "workflow" | "dashboard" | "industry_bundle" | "developer" | "ai_powered";
...
interface BuiltinTemplate {
  ...
  nodes: { id: string; name: string; type: string; nodeTypeId: string; position: { x: number; y: number }; config: Record<string, unknown> }[];
}
...
const BUILTIN_TEMPLATES: BuiltinTemplate[] = [
  ...
  // CRM workflows
  {
    ...
    description: "Capture website form submissions, validate leads, push them to HubSpot, notify your team on Slack, and send a personalised follow-up email.",
    category: "CRM",
    triggerType: "form",
    complexity: "intermediate",
    ...
    metadata: {},
    nodes: [
      { id: "lc1", name: "Website Form", type: "trigger", nodeTypeId: "trigger.form", position: { x: 60, y: 100 }, config: {} },
      { id: "lc2", name: "Lead Validation", type: "logic", nodeTypeId: "logic.if", position: { x: 300, y: 100 }, config: {} },
      { id: "lc3", name: "HubSpot CRM", type: "action", nodeTypeId: "action.hubspot", position: { x: 540, y: 100 }, config: { operation: "create_contact" } },
      { id: "lc4", name: "Slack Notification", type: "action", nodeTypeId: "action.slack", position: { x: 780, y: 60 }, config: { channel: "#sales" } },
      { id: "lc5", name: "Follow-up Email", type: "action", nodeTypeId: "action.send_email", position: { x: 780, y: 180 }, config: { subject: "Thanks for reaching out!" } },
    ],
  },
  {
    name: "Lead Scoring Workflow",
    description: "Enrich new leads with Clearbit data, score them based on company size and product fit, then auto-assign to the right sales rep in HubSpot or Salesforce.",
    category: "CRM",
    triggerType: "event",
    complexity: "advanced",
    ...
    metadata: {},
    nodes: [
      { id: "ls1", name: "New Lead", type: "trigger", nodeTypeId: "trigger.event", position: { x: 60, y: 100 }, config: {} },
      { id: "ls2", name: "Enrich Contact", type: "action", nodeTypeId: "action.clearbit", position: { x: 300, y: 100 }, config: {} },
      { id: "ls3", name: "Score Lead", type: "ai", nodeTypeId: "ai.score", position: { x: 540, y: 100 }, config: { criteria: "company_size,fit,intent" } },
      { id: "ls4", name: "Assign Sales Rep", type: "action", nodeTypeId: "action.hubspot", position: { x: 780, y: 100 }, config: { operation: "assign_owner" } },
    ],
  },
  ...
  // Communication workflows
  {
    ...
    description: "Classify incoming WhatsApp messages with AI, automatically create support tickets, and notify the right agent. Ideal for African SMBs where WhatsApp is dominant.",
    ...
    metadata: {},
    nodes: [
      { id: "wa1", name: "WhatsApp Message", type: "trigger", nodeTypeId: "trigger.webhook", position: { x: 60, y: 100 }, config: { source: "whatsapp" } },
      { id: "wa2", name: "AI Classification", type: "ai", nodeTypeId: "ai.classify", position: { x: 300, y: 100 }, config: { categories: "Billing,Technical,General" } },
      { id: "wa3", name: "Create Ticket", type: "action", nodeTypeId: "action.create_record", position: { x: 540, y: 100 }, config: { object_type: "Ticket" } },
      { id: "wa4", name: "Notify Agent", type: "action", nodeTypeId: "action.slack", position: { x: 780, y: 100 }, config: { channel: "#support-agents" } },
    ],
  },
  {
    name: "Missed Call Follow-Up",
    description: "When a phone call is missed, automatically send a WhatsApp message to the caller and create a CRM record so no lead falls through the cracks.",
    category: "Communication",
    triggerType: "event",
    complexity: "beginner",
    ...
    metadata: {},
    nodes: [
      { id: "mc1", name: "Phone Missed Call", type: "trigger", nodeTypeId: "trigger.event", position: { x: 60, y: 100 }, config: { event: "missed_call" } },
      { id: "mc2", name: "Send WhatsApp", type: "action", nodeTypeId: "action.whatsapp", position: { x: 300, y: 100 }, config: { template: "missed_call" } },
      { id: "mc3", name: "Create CRM Record", type: "action", nodeTypeId: "action.create_record", position: { x: 540, y: 100 }, config: { object_type: "Contact" } },
    ],
  },
  ...
  // Finance workflows
  {
    ...
    description: "Send escalating invoice reminders via email and WhatsApp when payment is overdue, with automatic escalation after 7 days.",
    category: "Finance",
    triggerType: "schedule",
    complexity: "intermediate",
    ...
    metadata: {},
    nodes: [
      { id: "ir1", name: "Invoice Due", type: "trigger", nodeTypeId: "trigger.schedule", position: { x: 60, y: 100 }, config: { interval: "daily" } },
      { id: "ir2", name: "Reminder Email", type: "action", nodeTypeId: "action.send_email", position: { x: 300, y: 80 }, config: { subject: "Invoice Payment Reminder" } },
      { id: "ir3", name: "WhatsApp Reminder", type: "action", nodeTypeId: "action.whatsapp", position: { x: 300, y: 180 }, config: { template: "invoice_reminder" } },
      { id: "ir4", name: "Escalate", type: "logic", nodeTypeId: "logic.if", position: { x: 540, y: 100 }, config: { condition: "days_overdue > 7" } },
    ],
  },
  {
    name: "Stripe Payment Success",
    description: "When a Stripe payment succeeds, generate a professional receipt, email it to the customer, and update the deal in your CRM automatically.",
    category: "Finance",
    triggerType: "webhook",
    complexity: "beginner",
    ...
    metadata: {},
    nodes: [
      { id: "sp1", name: "Stripe Payment", type: "trigger", nodeTypeId: "trigger.webhook", position: { x: 60, y: 100 }, config: { source: "stripe", event: "payment.succeeded" } },
      { id: "sp2", name: "Generate Receipt", type: "data", nodeTypeId: "data.transform", position: { x: 300, y: 100 }, config: {} },
      { id: "sp3", name: "Email Customer", type: "action", nodeTypeId: "action.send_email", position: { x: 540, y: 60 }, config: { subject: "Payment Received — Thank You!" } },
      { id: "sp4", name: "Update CRM", type: "action", nodeTypeId: "action.create_record", position: { x: 540, y: 180 }, config: { object_type: "Deal", operation: "update" } },
    ],
  },
  ...
  // HR workflows
  {
    ...
    description: "Automatically provision accounts, send a personalised welcome email with credentials, and assign onboarding tasks when a new employee is added.",
    category: "HR",
    triggerType: "event",
    complexity: "intermediate",
    ...
    metadata: {},
    nodes: [
      { id: "eo1", name: "New Employee", type: "trigger", nodeTypeId: "trigger.event", position: { x: 60, y: 100 }, config: { event: "employee.created" } },
      { id: "eo2", name: "Create Accounts", type: "action", nodeTypeId: "action.http", position: { x: 300, y: 100 }, config: { method: "POST" } },
      { id: "eo3", name: "Send Welcome Email", type: "action", nodeTypeId: "action.send_email", position: { x: 540, y: 60 }, config: { subject: "Welcome to the team!" } },
      { id: "eo4", name: "Create Tasks", type: "action", nodeTypeId: "action.create_record", position: { x: 540, y: 180 }, config: { object_type: "Task" } },
    ],
  },
  {
    name: "Leave Request Approval",
    description: "Route employee leave requests to their manager for approval, then automatically notify HR once a decision is made.",
    category: "HR",
    triggerType: "form",
    complexity: "beginner",
    ...
    metadata: {},
    nodes: [
      { id: "lr1", name: "Employee Request", type: "trigger", nodeTypeId: "trigger.form", position: { x: 60, y: 100 }, config: {} },
      { id: "lr2", name: "Manager Approval", type: "logic", nodeTypeId: "logic.approval", position: { x: 300, y: 100 }, config: { approver_role: "manager" } },
      { id: "lr3", name: "HR Notification", type: "action", nodeTypeId: "action.send_email", position: { x: 540, y: 100 }, config: { subject: "Leave Request Decision" } },
    ],
  },
...

  {
    ...
    description: "A full-featured CRM with lead tracking, kanban pipeline board, customer profiles, and a real-time activity feed. Connects to HubSpot, Salesforce, or PostgreSQL.",
    category: "Sales",
    triggerType: "manual",
    complexity: "intermediate",
    ...
    metadata: {
      components: ["Leads Table", "Pipeline Board", "Customer Profile", "Activity Feed"],
  ...
  },
  {
    ...
    description: "Manage support tickets, look up customers, track SLAs, and monitor agent performance from one unified view.",
    category: "Support",
    triggerType: "manual",
    complexity: "intermediate",
    ...
    templateType: "dashboard",
    metadata: {
      components: ["Ticket Queue", "Customer Lookup", "SLA Dashboard", "Agent Performance"],
  ...
  },
  {
    ...
    description: "Track products, monitor stock levels, manage suppliers, and handle purchase orders. Popular with retailers and distributors.",
    category: "Operations",
    triggerType: "manual",
    complexity: "beginner",
  ...
  },
  {
    ...
    description: "Manage property listings, track agent performance, review offers, schedule viewings, and monitor payments — all in one place.",
...

  {
    ...
    description: "Everything you need to run a real estate agency — lead routing, property approvals, viewing scheduling, offer management workflows, and a complete listings dashboard.",
    category: "Real Estate",
    triggerType: "manual",
    complexity: "advanced",
    ...
    metadata: {
      workflows: ["New Lead Routing", "Property Approval", "Viewing Scheduling", "Offer Management"],
  ...
  },
  {
    ...
    description: "Full e-commerce automation — order processing, payment confirmation, abandoned cart recovery, and refund handling with a complete orders and inventory dashboard.",
    category: "E-Commerce",
    triggerType: "manual",
    complexity: "advanced",
    ...
    templateType: "industry_bundle",
    metadata: {
      workflows: ["Order Received", "Payment Success", "Abandoned Cart", "Refund Request"],
  ...
  },
  {
    ...
    description: "Automate appointment reminders, patient follow-up notifications, and lab result alerts with a full patient management dashboard including billing.",
    category: "Healthcare",
    triggerType: "manual",
    complexity: "advanced",
    ...
    templateType: "industry_bundle",
    metadata: {
      workflows: ["Appointment Reminder", "Follow-up Notification", "Lab Result Alert"],
  ...
  },
  {
    ...
    description: "Manage student registrations, fee reminders, and result publications with dashboards for students, teachers, attendance, and payments. Especially useful in The Gambia.",
    category: "Education",
    triggerType: "manual",
    complexity: "advanced",
...

  {
    ...
    description: "A production-ready connector starter with OAuth 2.0 auth, automatic token refresh, and a sample action. Scaffold your integration in minutes.",
    category: "Connector",
    triggerType: "manual",
    complexity: "advanced",
  ...
  },
  {
    ...
    description: "Bootstrap a connector that authenticates via an API key header. Includes a sample GET action with error handling and response mapping.",
    category: "Connector",
    triggerType: "manual",
    complexity: "intermediate",
  ...
  },
  {
    ...
    description: "Start building a webhook-based connector with inbound signature verification, event parsing, and a trigger definition ready to go.",
    category: "Connector",
    triggerType: "manual",
    complexity: "intermediate",
  ...
  },
  {
    ...
    description: "A scaffold for a custom workflow action node. Handles input schema, config UI metadata, and async execution with structured error reporting.",
    category: "Node",
    triggerType: "manual",
    complexity: "intermediate",
  ...
  },
  {
    ...
    description: "Create a polling-based trigger that periodically checks an external API for new data and fires when something changes.",
    category: "Node",
    triggerType: "manual",
    complexity: "intermediate",
  ...
  },
  {
    ...
    description: "A production-ready data table for internal dashboards with sorting, filtering, pagination, and inline row actions.",
    category: "Component",
    triggerType: "manual",
    complexity: "beginner",
  ...
  },
  {
    ...
    description: "A configurable chart component for dashboards — supports line, bar, area, and pie charts with automatic data binding.",
    category: "Component",
    triggerType: "manual",
    complexity: "beginner",
  ...
  },
  {
    ...
    description: "A KPI metric card with trend indicator, comparison period, and configurable data source — drop into any dashboard.",
    category: "Component",
    triggerType: "manual",
    complexity: "beginner",
...

  {
    ...
    description: "AI-generated from: \"I run a real estate agency\" — full lead routing, viewing scheduling, offer approval, and payment reminder workflows with a complete properties dashboard.",
    category: "Real Estate",
    triggerType: "manual",
    complexity: "advanced",
    ...
    metadata: {
      ...
      generatedWorkflows: ["Lead Capture", "Viewing Scheduling", "Offer Approval", "Payment Reminder"],
  ...
  {
    name: "SaaS Startup",
    description: "AI-generated from: \"I run a SaaS startup\" — trial-to-paid conversion, churn prevention, NPS collection workflows with MRR, churn, and user dashboard.",
    category: "SaaS",
    triggerType: "manual",
    complexity: "advanced",
    ...
    metadata: {
      ...
      generatedWorkflows: ["Trial Signup", "Trial-to-Paid Nudge", "Churn Prevention", "NPS Survey"],
  ...
  {
    name: "Digital Agency",
    description: "AI-generated from: \"I run a digital marketing agency\" — client onboarding, report delivery, invoice reminders, and a project & revenue dashboard.",
    category: "Agency",
    triggerType: "manual",
    complexity: "intermediate",
    ...
    metadata: {
      ...
      generatedWorkflows: ["Client Onboarding", "Monthly Report Delivery", "Invoice Reminder", "Proposal Follow-up"],
...
async function ensureBuiltinTemplates() {
  const existing = await db
    .select({ id: templatesTable.id, templateType: templatesTable.templateType })
    .from(templatesTable)
  ...
  const typesToSeed = (
    ["workflow", "dashboard", "industry_bundle", "developer", "ai_powered"] as TemplateType[]
  ).filter((type) => !seededTypes.has(type));

  if (typesToSeed.length === 0) return;

  const toInsert = BUILTIN_TEMPLATES.filter((t) => typesToSeed.includes(t.templateType));
...
const CreateTemplateBody = z.object({
  ...
  templateType: z.enum(["workflow", "dashboard", "industry_bundle", "developer", "ai_powered"]).default("workflow"),
...
router.get("/templates", async (req, res): Promise<void> => {
  ...
  if ((params.data as Record<string, unknown>).templateType) {
    conditions.push(eq(templatesTable.templateType, (params.data as Record<string, unknown>).templateType as string));
  }
...
router.post("/templates", async (req, res): Promise<void> => {
  ...
  const [template] = await db
    ...
    .values({ ...rest, nodes, metadata, nodeCount: nodes.length, isCustom: true })
    .returning();
