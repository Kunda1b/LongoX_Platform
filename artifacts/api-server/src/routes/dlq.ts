...
import { db, dlqEntriesTable, workflowsTable, executionsTable } from "@workspace/db";
...
router.get("/dlq", async (req, res): Promise<void> => {
  ...
  if (resolvedParam === "true") {
    entries = await db.select().from(dlqEntriesTable).where(isNotNull(dlqEntriesTable.resolvedAt)).orderBy(dlqEntriesTable.createdAt).limit(limit);
  } else if (resolvedParam === "false") {
    entries = await db.select().from(dlqEntriesTable).where(isNull(dlqEntriesTable.resolvedAt)).orderBy(dlqEntriesTable.createdAt).limit(limit);
  } else {
    entries = await db.select().from(dlqEntriesTable).orderBy(dlqEntriesTable.createdAt).limit(limit);
  }
...
router.post("/dlq/:id/retry", async (req, res): Promise<void> => {
  ...
  const [entry] = await db.select().from(dlqEntriesTable).where(eq(dlqEntriesTable.id, id));
  if (!entry) { res.status(404).json({ error: "DLQ entry not found" }); return; }

  const [workflow] = await db.select().from(workflowsTable).where(eq(workflowsTable.id, entry.workflowId));
  ...
  await db.update(dlqEntriesTable)
...
router.post("/dlq/:id/dismiss", async (req, res): Promise<void> => {
  ...
  const [updated] = await db.update(dlqEntriesTable)
