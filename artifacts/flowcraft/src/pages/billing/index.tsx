...
export default function BillingPage() {
  ...
  const maxQty = Math.max(
    ...(current?.usageBreakdown ?? []).map((l) => l.quantity),
    1,
  );
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          value={
            loadingSum
              ? "–"
              : (summary?.executionsThisMonth ?? 0).toLocaleString()
          }
        ...
        <StatCard
          ...
          value={
            loadingSum
              ? "–"
              : `$${(summary?.currentPeriodCost ?? 0).toFixed(2)}`
          }
          sub={
            summary?.budgetLimit ? `$${summary.budgetLimit} budget` : undefined
          }
          icon={DollarSign}
          accent="text-primary"
        />
          ...
          <CardContent className="space-y-4">
            {(current?.usageBreakdown ?? []).length === 0 ? (
              ...
              current?.usageBreakdown.map((item) => {
                const cfg = EVENT_LABELS[item.label] ?? {
                  label: item.label,
                  ...
                  <div key={item.label} className="space-y-1.5">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <div className="flex items-center gap-3 text-muted-foreground">
                        <span>
                          {item.quantity.toLocaleString()} × ${item.unitCost}
                        </span>
            ...
            <ScrollArea className="h-[300px]">
              {loadingEvents ? (
                ...
                <div className="divide-y">
                  {events.map((e) => {
                    const cfg = EVENT_LABELS[e.eventType] ?? {
                      label: e.eventType,
                      color: "bg-muted",
                    };
                    return (
                  ...
                  <div key={inv.id} className="py-3 flex items-center gap-4">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        {inv.lineItems
                          ?.map((l) => EVENT_LABELS[l.label]?.label ?? l.label)
                          .join(" · ")}
                      </p>
                    </div>
                    <div
                      className={`flex items-center gap-1.5 text-sm ${st.className}`}
                    >
                      <StatusIcon size={14} />
                      <span>{st.label}</span>
                    </div>
