...
import type { Connector } from "@longox/api-client-react";
    ...
    <div className="space-y-1.5">
      {entries.map(([key, def]) => (
        <div key={key} className="flex items-start gap-2 text-xs">
          <code className="bg-muted px-1.5 py-0.5 rounded text-[0.7rem] font-mono shrink-0">
            {key}
          </code>
          ...
          {required.includes(key) && (
            <span className="text-red-500 text-[0.65rem] font-medium">
              required
            </span>
          )}
          {!!def.description && (
...
function ConnectorDetailDialog({
  ...
  const { data: triggers = [] } = useQuery<ConnectorTrigger[]>({
    ...
    queryFn: () =>
      fetch(`/api/connectors/${connector.id}/triggers`).then((r) => r.json()),
  });
  ...
  const { data: executions = [] } = useQuery<ConnectorExecution[]>({
    ...
    queryFn: () =>
      fetch(
        `/api/connectors/executions?connectorId=${connector.id}&limit=20`,
      ).then((r) => r.json()),
  });

  const cert =
    CERT_CONFIG[connector.certificationLevel as keyof typeof CERT_CONFIG] ??
    CERT_CONFIG.community;
  const auth =
    AUTH_CONFIG[connector.authType as keyof typeof AUTH_CONFIG] ??
    AUTH_CONFIG.apikey;
  ...
  const avgLatency = executions.length
    ? Math.round(
        executions.reduce((s, e) => s + (e.durationMs ?? 0), 0) /
          executions.length,
      )
    : null;
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                  <Badge
                    variant="outline"
                    className={`text-[0.65rem] px-1.5 py-0.5 flex items-center gap-1 ${cert.color}`}
                  >
                  ...
                  <Badge
                    variant="outline"
                    className={`text-[0.65rem] px-1.5 py-0.5 flex items-center gap-1 ${auth.color}`}
                  >
                  ...
                  <div className="flex items-center gap-1 text-xs text-muted-foreground">
                    <div
                      className={`w-2 h-2 rounded-full ${getHealthDot(connector.healthStatus)}`}
                    />
                    {connector.healthStatus?.availability
                      ? `${connector.healthStatus.availability}% uptime`
                      : "Status unknown"}
                  </div>
                </div>
              </div>
        ...
        >
          <div className="overflow-x-auto border-b">
            <TabsList className="px-4 sm:px-6 justify-start rounded-none bg-transparent h-10 gap-1 w-max min-w-full">
              <TabsTrigger
                value="overview"
                className="text-xs data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none whitespace-nowrap"
              >
                Overview
              </TabsTrigger>
              <TabsTrigger
              ...
              >
                Actions{" "}
                <Badge className="ml-1 text-[0.6rem] px-1 py-0">
                  {actions.length}
                </Badge>
              </TabsTrigger>
              <TabsTrigger
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            <TabsContent value="overview" className="p-6 space-y-5 mt-0">
              ...
              {connector.authConfig &&
                Object.keys(connector.authConfig).length > 0 && (
                  <div>
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">
                      Auth Configuration
                    </p>
                  ...
                  >
                    ...
                    {expandedAction === action.actionId && (
                      <div className="px-4 pb-4 pt-2 border-t bg-muted/20 grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          <p className="text-[0.65rem] font-semibold text-muted-foreground uppercase tracking-wider mb-2">
                            Input Schema
                          </p>
              ...
              {triggers.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Use actions to interact with this service.
                  </p>
                </div>
              ) : (
                  ...
                  <div className="space-y-2">
                    {connector.healthStatus.apiErrorRate !== undefined && (
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span
                          className={`font-medium ${Number(connector.healthStatus.apiErrorRate) > 0.01 ? "text-red-500" : "text-emerald-600"}`}
                        >
                          {(
                            Number(connector.healthStatus.apiErrorRate) * 100
                          ).toFixed(2)}
                          %
                        </span>
                      </div>
                    ...
                    {connector.healthStatus.oauthFailureRate !== undefined && (
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span
                          className={`font-medium ${Number(connector.healthStatus.oauthFailureRate) > 0.01 ? "text-red-500" : "text-emerald-600"}`}
                        >
                          {(
                            Number(connector.healthStatus.oauthFailureRate) *
                            100
                          ).toFixed(2)}
                          %
                        </span>
                    ...
                    {connector.healthStatus.avgLatencyMs !== undefined && (
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="font-medium">
                          {String(connector.healthStatus.avgLatencyMs)}ms
                        </span>
                      </div>
                    )}
                  </div>
                ...
                <div>
                  ...
                  <div className="space-y-1.5">
                    {executions.slice(0, 8).map((ex) => (
                      ...
                      >
                        <div
                          className={`w-1.5 h-1.5 rounded-full shrink-0 ${ex.status === "success" ? "bg-emerald-500" : "bg-red-500"}`}
                        />
                        <span className="text-muted-foreground font-mono truncate">
                          {ex.actionId ?? ex.triggerId ?? "—"}
                        </span>
                        <span className="text-muted-foreground ml-auto">
                          {ex.durationMs ? `${ex.durationMs}ms` : "—"}
                        </span>
                        <span
                          className={`capitalize ${ex.status === "success" ? "text-emerald-600" : "text-red-500"}`}
                        >
                          {ex.status}
                        </span>
                      </div>
...
function ConnectorCard({
  ...
  const cert =
    CERT_CONFIG[conn.certificationLevel as keyof typeof CERT_CONFIG] ??
    CERT_CONFIG.community;
  const auth =
    AUTH_CONFIG[conn.authType as keyof typeof AUTH_CONFIG] ??
    AUTH_CONFIG.apikey;
  const CertIcon = cert.icon;
  const AuthIcon = auth.icon;
    ...
    >
      <CardContent className="p-5 flex-1 flex flex-col gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                <div
                  className={`w-1.5 h-1.5 rounded-full shrink-0 ${healthDot}`}
                />
              </div>
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          <Badge
            variant="outline"
            className={`text-[0.6rem] px-1.5 py-0.5 flex items-center gap-0.5 ${cert.color}`}
          >
            <CertIcon className="h-2.5 w-2.5" />
            {cert.label}
          </Badge>
          <Badge
            variant="outline"
            className={`text-[0.6rem] px-1.5 py-0.5 flex items-center gap-0.5 ${auth.color}`}
          >
            <AuthIcon className="h-2.5 w-2.5" />
            {auth.label}
          </Badge>
          {!!(caps as Record<string, unknown>).webhookTriggers && (
            <Badge
              variant="outline"
              className="text-[0.6rem] px-1.5 py-0.5 bg-violet-50 text-violet-700 border-violet-200 flex items-center gap-0.5"
            >
          ...
          {!!(caps as Record<string, unknown>).pagination && (
            <Badge
              variant="outline"
              className="text-[0.6rem] px-1.5 py-0.5 bg-sky-50 text-sky-700 border-sky-200"
            >
              Pagination
            </Badge>
          )}
...
export default function Connectors() {
    ...
    mutation: {
      onSuccess: (data) => {
        ...
        toast({
          ...
          description: `${data.name} is ready to use in workflows.`,
        });
  ...
  async function handleUninstall(id: number) {
    ...
    try {
      const res = await fetch(`/api/connectors/${id}/uninstall`, {
        method: "POST",
      });
    ...
    <div className="flex h-full gap-0 max-w-[1400px] mx-auto w-full">
        ...
        <div className="mt-auto px-4 pt-4 border-t">
          <div className="bg-muted/50 rounded-lg p-3 space-y-2">
            <div className="flex justify-between text-xs">
              ...
              <span className="font-medium">
                {installedCount} / {connectors.length}
              </span>
            </div>
            <div className="w-full bg-muted rounded-full h-1.5">
              <div
                ...
                style={{
                  width: connectors.length
                    ? `${(installedCount / connectors.length) * 100}%`
                    : "0%",
                }}
              />
            </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-0.5">
                Production-grade integrations with full action &amp; trigger
                support
              </p>
            </div>
            ...
            <div className="flex flex-col items-center justify-center gap-4 text-center py-20 border border-dashed rounded-xl">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  Try a different category or certification filter.
                </p>
              </div>
            </div>
          ) : (
      ...
      {selectedConnector && (
        <ConnectorDetailDialog
          ...
          installing={
            installingId === selectedConnector.id ||
            uninstallingId === selectedConnector.id
          }
        />
      )}
    </div>
