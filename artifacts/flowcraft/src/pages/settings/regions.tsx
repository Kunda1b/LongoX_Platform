...
import {
  useListRegionPolicies,
  useCreateRegionPolicy,
  useUpdateRegionPolicy,
  useDeleteRegionPolicy,
  getListRegionPoliciesQueryKey,
} from "@longox/api-client-react";
import type { RegionPolicy } from "@longox/api-client-react";
...
export default function RegionsPage() {
    ...
    mutation: {
      onSuccess: () => {
        queryClient.invalidateQueries({
          queryKey: getListRegionPoliciesQueryKey(),
        });
        toast({ title: "Region policy created!" });
        setCreateOpen(false);
        setForm(EMPTY);
      },
      onError: () =>
  ...
  function openEdit(p: RegionPolicy) {
    setForm({
      ...
      region: p.region,
      tier: p.tier,
      replicationFactor: String(p.replicationFactor),
      notes: p.notes ?? "",
    });
    setEditTarget(p);
  }
  ...
  function handleCreate() {
    createMutation.mutate({
      data: {
        ...
        tier: form.tier as "standard" | "premium" | "edge",
        replicationFactor: Number(form.replicationFactor),
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Define region policies, replication factors, and deployment tiers.
          </p>
        </div>
        <Button
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          {
            label: "Premium",
            value: policies.filter((p) => p.tier === "premium").length,
          ...
          },
          {
            ...
            value:
              policies.length > 0
                ? (
                    policies.reduce((s, p) => s + p.replicationFactor, 0) /
                    policies.length
                  ).toFixed(1)
                : "—",
            color: "text-blue-600",
            ...
            <Card key={p.id}>
              ...
              <CardContent className="pt-0">
                <div className="flex items-center gap-2 flex-wrap mb-3">
                  <Badge
                    className={`text-[10px] uppercase font-semibold px-1.5 ${TIER_STYLES[p.tier] ?? ""}`}
                  >
                    {p.tier}
                  </Badge>
                  <div className="flex items-center gap-1 text-xs text-muted-foreground">
                    <Server className="h-3 w-3" />
                    <span>×{p.replicationFactor} replicas</span>
                  </div>
                </div>
                {p.notes && (
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="space-y-1.5">
                <Label>Notes</Label>
                <Textarea
                  placeholder="Architecture notes, SLA requirements…"
                  rows={3}
                  value={form.notes}
                  onChange={(e) =>
            ...
            <DialogFooter>
              ...
              <Button
                onClick={onSubmit}
                disabled={!form.name.trim() || !form.region.trim() || isPending}
              >
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete this region policy?</AlertDialogTitle>
            <AlertDialogDescription>
