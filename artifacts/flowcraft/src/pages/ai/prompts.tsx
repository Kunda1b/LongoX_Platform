...
import type { Prompt } from "@longox/api-client-react";
...
export default function PromptsPage() {
  ...
  const { data: versions = [] } = useListPromptVersions(
    versionTarget?.id ?? 0,
    { query: { enabled: versionTarget !== null, queryKey: [] } },
  );
  ...
  const publishMutation = usePublishPrompt({
    mutation: {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: getListPromptsQueryKey() });
        toast({ title: "Prompt published!" });
      },
      onError: () =>
        toast({ title: "Failed to publish prompt", variant: "destructive" }),
    },
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Version-controlled prompt assets for your AI workflows.
          </p>
        </div>
        <Button
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Draft", value: draftCount, color: "text-slate-600" },
          { label: "In Review", value: reviewCount, color: "text-amber-600" },
          ...
          {prompts.map((p) => (
            <Card key={p.id} className="flex flex-col">
              ...
              <CardContent className="pt-0 flex-1 flex flex-col gap-3">
                <pre className="text-xs text-muted-foreground bg-muted/40 rounded p-2 overflow-hidden line-clamp-3 whitespace-pre-wrap font-mono">
                  {p.content}
                </pre>
                <div className="flex items-center gap-2 flex-wrap mt-auto">
                  <Badge
                    className={`text-[10px] uppercase font-semibold px-1.5 ${STATUS_STYLES[p.status] ?? ""}`}
                  >
                    {p.status}
                  </Badge>
                  <span className="text-xs text-muted-foreground flex items-center gap-1">
                    <Clock className="h-3 w-3" />v{p.version}
                  </span>
          ...
          <DialogContent className="sm:max-w-2xl">
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="space-y-1.5">
                <Label>Tags</Label>
                <Input
                  placeholder="summarize, json, crm (comma-separated)"
      ...
      >
        <SheetContent className="w-full sm:max-w-md overflow-y-auto">
          <SheetHeader>
            <SheetTitle>Version History — {versionTarget?.name}</SheetTitle>
          </SheetHeader>
          <div className="mt-6 space-y-4">
            {versions.map((v) => (
              <div key={v.id} className="border rounded-lg p-3 space-y-2">
                <div className="flex items-center gap-2">
                  <Badge
                    className={`text-[10px] uppercase font-semibold ${STATUS_STYLES[v.status] ?? ""}`}
                  >
                    {v.status}
                  </Badge>
                ...
                <pre className="text-xs text-muted-foreground bg-muted/40 rounded p-2 overflow-auto max-h-32 whitespace-pre-wrap font-mono">
                  {v.content}
                </pre>
                {v.notes && (
