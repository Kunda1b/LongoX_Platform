import { useGetDashboardSummary, useGetRecentActivity } from "@autoflow/api-client-react";
          ...
          <CardContent>
            {isLoadingActivity ? (
              ...
              <div className="space-y-3">
                {activity.map((item) => (
                  <div key={item.id} className="flex items-center gap-3 p-3 sm:p-4 rounded-lg border bg-card hover:bg-accent/50 transition-colors">
                    {/* Status icon */}
                    <div className={`p-2 rounded-full shrink-0 ${
                      ...
                      {item.status === 'success'   && <CheckCircle2 className="h-3.5 w-3.5 sm:h-4 sm:w-4" />}
                      {item.status === 'failed'    && <XCircle className="h-3.5 w-3.5 sm:h-4 sm:w-4" />}
                      {item.status === 'running'   && <PlayCircle className="h-3.5 w-3.5 sm:h-4 sm:w-4 animate-pulse" />}
                      {item.status === 'cancelled' && <Settings2 className="h-3.5 w-3.5 sm:h-4 sm:w-4" />}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground flex gap-2 mt-0.5">
                        <span className="truncate">{formatDistanceToNow(new Date(item.startedAt), { addSuffix: true })}</span>
                        {item.durationMs && <span className="shrink-0">· {item.durationMs}ms</span>}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 sm:gap-4 shrink-0">
                      ...
                      <Link href={`/executions/${item.id}`} className="text-xs text-primary font-medium hover:underline hidden sm:inline">
                        Details
                      </Link>
                    </div>
                  </div>
  ...
  return (
    <Card>
      ...
      <CardContent className="px-4 pb-4 sm:px-6 sm:pb-6">
        ...
        {description && <p className="text-xs text-muted-foreground mt-1 hidden sm:block">{description}</p>}
      </CardContent>
    </Card>
  );
}
