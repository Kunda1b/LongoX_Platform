...
export async function createRecord(
  ...
  return {
    success: !!result.id,
    data: { id: String(result.id ?? ""), success: !!result.id },
    error: result.id ? null : "Failed to create record",
    durationMs: Date.now() - start,
  };
}
...
export async function queryRecords(
  ...
  const instanceUrl = (context.auth.credentials.instanceUrl as string) ?? "";
  const query = String(
    context.config.query ?? "SELECT Id, Name FROM Account LIMIT 10",
  );
  ...
  return {
    success: true,
    data: {
      records: (result.records as unknown[]) ?? [],
