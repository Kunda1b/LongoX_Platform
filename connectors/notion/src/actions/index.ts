...
export async function createPage(
  ...
  const properties = (context.config.properties ?? {}) as Record<
    string,
    unknown
  >;
  ...
  if (!token)
    return {
      success: false,
      data: {},
      error: "Notion API token required",
  ...
  try {
    ...
    return {
      ...
      data: {
        id: data.id,
        url: data.url,
        createdTime: (data.created_time as string) ?? new Date().toISOString(),
      },
      error: null,
      durationMs: Date.now() - start,
    };
  ...
  try {
    ...
    return {
      ...
      data: {
        ...
        url: data.url,
        updatedTime:
          (data.last_edited_time as string) ?? new Date().toISOString(),
      },
      error: null,
      durationMs: Date.now() - start,
    };
