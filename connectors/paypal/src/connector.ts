...
export const paypalConnector: ConnectorDefinition = {
  ...
  actions: [
    ...
    },
    {
      id: "createProduct",
      name: "Create Product",
      description: "Create a PayPal product",
      inputSchema: { name: "string", description: "string?", type: "string?" },
    ...
    },
    {
      ...
      inputSchema: { planId: "string", subscriberEmail: "string" },
      outputSchema: { id: "string", status: "string" },
      idempotent: false,
    },
  ],
  triggers: [
    {
      ...
      description: "Trigger when a payment completes",
      type: "webhook",
      outputSchema: { orderId: "string", amount: "number", status: "string" },
    },
    {
...
connectorRegistry.registerAction("paypal", {
  definition: paypalConnector.actions[0],
  handler: createOrder,
});
connectorRegistry.registerAction("paypal", {
  definition: paypalConnector.actions[1],
  handler: captureOrder,
});
connectorRegistry.registerAction("paypal", {
  definition: paypalConnector.actions[2],
  handler: createProduct,
});
connectorRegistry.registerAction("paypal", {
  definition: paypalConnector.actions[3],
  handler: createSubscription,
});
connectorRegistry.registerTrigger("paypal", {
  definition: paypalConnector.triggers[0],
  handler: paymentCompleted,
});
connectorRegistry.registerTrigger("paypal", {
  definition: paypalConnector.triggers[1],
