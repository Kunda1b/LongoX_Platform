...
export const hubspotConnector: ConnectorDefinition = {
  ...
  actions: [
    ...
    },
    {
      ...
      inputSchema: { name: "string", domain: "string?" },
  ...
  triggers: [
    {
      ...
      type: "webhook",
      outputSchema: { contactId: "string", email: "string", changes: "object" },
    },
    {
      id: "dealUpdated",
      name: "Deal Updated",
      description: "Trigger when a deal stage changes",
      type: "webhook",
      outputSchema: { dealId: "string", dealName: "string", stage: "string" },
    },
  ],
  permissions: [
    "contacts.read",
...
connectorRegistry.registerAction("hubspot", {
  definition: hubspotConnector.actions[0],
  handler: createContact,
});
connectorRegistry.registerAction("hubspot", {
  definition: hubspotConnector.actions[1],
  handler: createCompany,
});
connectorRegistry.registerAction("hubspot", {
  definition: hubspotConnector.actions[2],
  handler: createDeal,
});
connectorRegistry.registerTrigger("hubspot", {
  definition: hubspotConnector.triggers[0],
  handler: contactUpdated,
});
connectorRegistry.registerTrigger("hubspot", {
  definition: hubspotConnector.triggers[1],
  handler: dealUpdated,
});
