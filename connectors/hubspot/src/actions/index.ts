...
export async function createContact(
  ...
  const { email, firstName, lastName, phone } = context.config as Record<
    string,
    string
  >;
  ...
  if (!data.id)
    return {
      success: false,
      data: {},
      error: String(data.message ?? "HubSpot API error"),
      durationMs: Date.now() - start,
    };

  return {
    success: true,
    data: { id: String(data.id), email },
    error: null,
    durationMs: Date.now() - start,
  };
}
...
export async function createDeal(
  ...
  const start = Date.now();
  const token = (context.auth.credentials.accessToken as string) ?? "";
  const { dealName, amount, dealStage } = context.config as Record<
    string,
    string
  >;
  ...
  return {
    success: true,
    data: { id: String(data.id ?? ""), dealName },
    error: null,
    durationMs: Date.now() - start,
  };
}
