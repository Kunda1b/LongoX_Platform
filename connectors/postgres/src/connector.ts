...
connectorRegistry.registerAction("postgres", {
  definition: postgresConnector.actions[0],
  handler: executeQuery,
});
connectorRegistry.registerAction("postgres", {
  definition: postgresConnector.actions[1],
  handler: listTables,
});
connectorRegistry.registerAction("postgres", {
  definition: postgresConnector.actions[2],
  handler: getTableSchema,
});
connectorRegistry.registerTrigger("postgres", {
  definition: postgresConnector.triggers[0],
  handler: rowInserted,
});
connectorRegistry.registerTrigger("postgres", {
  definition: postgresConnector.triggers[1],
  handler: rowUpdated,
});
connectorRegistry.registerTrigger("postgres", {
  definition: postgresConnector.triggers[2],
  handler: rowDeleted,
});
