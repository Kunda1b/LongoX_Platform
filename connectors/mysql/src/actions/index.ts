...
async function queryMysql(
  ...
  try {
    ...
    return {
      ...
      fields: (fields as any[])?.map((f: any) => f.name ?? f.orgName ?? String(f)) ?? [],
    };
  } finally {
...
export async function executeQuery(context: ActionContext): Promise<ActionResult> {
  ...
  const raw = (context.config.connectionString as string) ?? (context.auth.credentials as Record<string, string>)?.connectionString ?? "";
  const query = String(context.config.query ?? "");
  const params = (context.config.params as unknown[]) ?? [];

  if (!raw) return { success: false, data: {}, error: "Connection string required", durationMs: Date.now() - start };
...
export async function listTables(context: ActionContext): Promise<ActionResult> {
  ...
  try {
    const config = parseConnectionString(raw);
    const result = await queryMysql(config, "SELECT TABLE_NAME, TABLE_TYPE, TABLE_SCHEMA FROM information_schema.tables WHERE TABLE_SCHEMA = ? ORDER BY TABLE_NAME", [config.database]);
    return {
      success: true,
      data: {
...
export async function getTableSchema(context: ActionContext): Promise<ActionResult> {
  ...
  if (!raw) return { success: false, data: {}, error: "Connection string required", durationMs: Date.now() - start };
  if (!table) return { success: false, data: {}, error: "table name required", durationMs: Date.now() - start };

  try {
    const config = parseConnectionString(raw);
    const result = await queryMysql(config, "SELECT COLUMN_NAME, DATA_TYPE, IS_NULLABLE, COLUMN_DEFAULT, CHARACTER_MAXIMUM_LENGTH FROM information_schema.columns WHERE TABLE_NAME = ? AND TABLE_SCHEMA = ? ORDER BY ORDINAL_POSITION", [table, config.database]);
    return {
      success: true,
      data: {
