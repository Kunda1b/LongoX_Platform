...
export const listDataSources = async (
  ...
  return customFetch<DataSource[]>(
    `${getListDataSourcesUrl()}${qs ? `?${qs}` : ""}`,
    { ...options, method: "GET" },
  );
};
...
export const listAdapters = async (
  ...
  return customFetch<string[]>(getListAdaptersUrl(), {
    ...options,
    method: "GET",
  });
};
...
export const getListAdaptersQueryKey = () =>
  [`/api/datasources/adapters`] as const;
...
export function useListAdapters<
  ...
  TError = ErrorType<unknown>,
>(options?: {
  query?: UseQueryOptions<string[], TError, TData>;
  request?: RequestInit;
}) {
  const { query: queryOptions, request: requestOptions } = options ?? {};
