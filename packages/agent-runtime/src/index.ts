...
import type { AgentConfig, AgentContext, AgentRunResult, AgentMessage, AgentToolCall, AgentToolResult } from "./types";
export * from "./types";
export class AgentRuntime {
  ...
  async run(context: AgentContext): Promise<AgentRunResult> {
    ...
    const messages: AgentMessage[] = [
      { id: randomUUID(), role: "system", content: this.config.systemPrompt, timestamp: new Date().toISOString() },
      ...context.history,
    ];
    ...
    try {
      while (iterationCount < this.config.maxIterations) {
        ...
        if (iterationCount >= this.config.maxIterations || assistantMsg.content.includes("FINAL")) {
    ...
    return {
      success: !error, output: finalOutput ?? error, messages,
      iterationCount, durationMs,
