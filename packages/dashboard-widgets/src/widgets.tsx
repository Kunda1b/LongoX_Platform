...
function extractKey(rows: Record<string, unknown>[], preferredKey?: string): string {
...
export function TimeSeriesChartWidget({ widget, data }: WidgetComponentProps) {
  ...
  if (rows.length === 0) {
    return <div className="flex h-48 items-center justify-center text-sm text-muted-foreground rounded-lg border bg-card">No data</div>;
  }
  const key = extractKey(rows, config.xKey);
  const valKey = config.yKey ?? (Object.keys(rows[0]).find(k => k !== key && typeof rows[0][k] === "number") || "value");
    ...
    <div className="rounded-lg border bg-card p-4">
      <ResponsiveContainer width="100%" height={200}>
        <LineChart data={rows}>
          ...
          <XAxis dataKey={key} tick={{ fontSize: 12 }} stroke="hsl(var(--muted-foreground))" />
          <YAxis tick={{ fontSize: 12 }} stroke="hsl(var(--muted-foreground))" />
          <Tooltip />
          <Line type="monotone" dataKey={valKey} stroke="#6366f1" strokeWidth={2} dot={{ r: 3 }} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
...
export function DataTableWidget({ widget, data }: WidgetComponentProps) {
  const rows = isRecordArray(data) ? data : [];
  if (rows.length === 0) {
    return <div className="flex h-32 items-center justify-center text-sm text-muted-foreground rounded-lg border bg-card">No data</div>;
  }
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b bg-muted/50">
            {columns.map(col => (
              <th key={col} className="px-3 py-2 text-left font-medium text-muted-foreground text-xs uppercase tracking-wider">{col}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} className="border-b last:border-0 hover:bg-muted/30">
              {columns.map(col => (
                <td key={col} className="px-3 py-2">{formatValue(row[col])}</td>
              ))}
            </tr>
          ))}
        </tbody>
    ...
    <div className="flex gap-3 overflow-x-auto rounded-lg border bg-card p-3">
      {Array.from(groups.entries()).map(([group, groupItems]) => (
        <div key={group} className="min-w-48 flex-1">
          <h4 className="mb-2 text-xs font-semibold uppercase text-muted-foreground">{group} ({groupItems.length})</h4>
          <div className="space-y-2">
            {groupItems.map((item, i) => {
              const title = config.titleKey ? String(item[config.titleKey] ?? "") : String(item.id ?? i);
              return (
                <div key={i} className="rounded-md border bg-background p-2 text-sm shadow-sm">
                  {title}
                </div>
              );
            })}
...
export function AiInsightCardWidget({ widget, data }: WidgetComponentProps) {
  const insight = typeof data === "string" ? data : isRecord(data) ? String(data.insight ?? data.text ?? "") : "No insight available";
  return (
    <div className="rounded-lg border bg-card p-4">
    ...
    <div className="space-y-2 rounded-lg border bg-card p-4">
      ...
      {entries.length === 0 ? (
        ...
        entries.slice(0, 10).map((entry, i) => (
          <div key={i} className="flex items-center justify-between border-b pb-1 last:border-0 text-xs">
            <span>{String(entry.action ?? entry.type ?? "—")}</span>
            <span className="text-muted-foreground">{String(entry.timestamp ?? entry.createdAt ?? "")}</span>
          </div>
        ))
      )}
    </div>
    ...
    <div className="rounded-lg border bg-card p-4">
      ...
      {tasks.length === 0 ? (
        ...
        <div className="space-y-1">
          {tasks.map((task, i) => (
            <div key={i} className="flex items-center gap-2 rounded-md border bg-background px-3 py-2 text-xs">
              <span className="flex-1">{String(task.name ?? task.title ?? `Task ${i + 1}`)}</span>
...
export function renderWidget(widget: WidgetInstance, context: RenderContext): React.ReactNode {
  const Component = widgetRegistry[widget.type];
  if (!Component) {
    return <div className="flex h-32 items-center justify-center rounded-lg border bg-muted text-sm text-muted-foreground">Unknown widget type: {widget.type}</div>;
  }
