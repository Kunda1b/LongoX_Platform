...
function PreviewNode({ data }: { data: any }) {
  ...
  return (
    ...
    >
      ...
      >
        <span className={cn("p-1 rounded shrink-0", meta.bg, meta.color)}>
          {meta.icon}
        </span>
        ...
        <Badge
          variant="outline"
          className={cn(
            "text-[9px] px-1.5 py-0 uppercase tracking-wide shrink-0",
            meta.color,
            meta.border,
          )}
        >
...
function deriveCategory(nodeTypeId: string): string {
  ...
  if (["trigger", "action", "logic", "ai", "data"].includes(prefix))
    return prefix;
  return "action";
}
...
function PreviewContent({
  ...
  return (
    <div className="flex flex-1 min-h-0 gap-0">
      {/* ─── Graph canvas ──────────────────────────────────────────────────── */}
      <div
        className="flex-1 min-w-0 border-r bg-muted/20 rounded-bl-lg"
        style={{ height: 520 }}
      >
        {nodes.length === 0 ? (
          ...
          >
            ...
            <MiniMap
              nodeColor={(n) =>
                HANDLE_COLORS[(n.data?.category as string) ?? "action"] ??
                "#6b7280"
              }
...
export function TemplatePreviewDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent
        className="max-w-4xl p-0 gap-0 overflow-hidden flex flex-col"
        style={{ height: 620 }}
      >
