...
import type { WidgetType, WidgetInstance, DashboardDefinition, DashboardLayout } from "@longox/dashboard-renderer";
...
const WIDGET_CATALOG: { type: WidgetType; label: string; icon: React.ReactNode; description: string }[] = [
  ...
  { type: "time-series-chart", label: "Time Series", icon: <Activity className="h-4 w-4" />, description: "Line chart over time" },
  { type: "data-table", label: "Data Table", icon: <Table2 className="h-4 w-4" />, description: "Tabular data view" },
  { type: "kanban-board", label: "Kanban Board", icon: <Kanban className="h-4 w-4" />, description: "Card-based column view" },
  { type: "form-panel", label: "Form Panel", icon: <FormInput className="h-4 w-4" />, description: "Inline form" },
  { type: "map-widget", label: "Map", icon: <Map className="h-4 w-4" />, description: "Geographic visualization" },
  { type: "file-widget", label: "File Upload", icon: <FileText className="h-4 w-4" />, description: "File attachment panel" },
  { type: "ai-insight-card", label: "AI Insight", icon: <BrainCircuit className="h-4 w-4" />, description: "AI-generated analysis" },
  { type: "audit-feed", label: "Audit Feed", icon: <ListChecks className="h-4 w-4" />, description: "Recent activity stream" },
  { type: "task-queue", label: "Task Queue", icon: <Activity className="h-4 w-4" />, description: "Approval task list" },
];
...
export function DashboardBuilder({ existingDashboard }: DashboardBuilderProps) {
  ...
  const [widgets, setWidgets] = useState<WidgetInstance[]>(existingDashboard?.widgets ?? []);
  ...
  const handleSave = () => {
    if (existingDashboard?.id) {
      updateMutation.mutate({ id: parseInt(existingDashboard.id), data: { name: title, widgets: widgets as any } });
    } else {
      createMutation.mutate({ data: { name: title, widgets: widgets as any } as any });
    }
  };

  const handlePublish = () => {
    if (!existingDashboard?.id) {
      toast({ title: "Save the dashboard first before publishing", variant: "destructive" });
      return;
    }
    publishMutation.mutate({ id: parseInt(existingDashboard.id), data: { notes: publishNotes } as any });
  };
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] gap-0">
      ...
      <div className={cn("border-r bg-card transition-all", sidebarOpen ? "w-64" : "w-0 overflow-hidden")}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex items-center justify-between border-b px-4 h-12 shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Eye className="mr-1 h-4 w-4" /> {previewMode ? "Edit" : "Preview"}
            </Button>
            <Button variant="outline" size="sm" onClick={handleSave} disabled={createMutation.isPending || updateMutation.isPending}>
              <Save className="mr-1 h-4 w-4" /> Save
            </Button>
            <Button size="sm" onClick={() => setIsPublishDialogOpen(true)} disabled={!existingDashboard?.id}>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={v => updateWidget(selectedWidget.id, { position: { ...selectedWidget.position, w: parseInt(v) } })}
              >
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={v => updateWidget(selectedWidget.id, { position: { ...selectedWidget.position, h: parseInt(v) } })}
              >
                <SelectTrigger className="h-8">
                  <SelectValue />
                </SelectTrigger>
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={v => updateWidget(selectedWidget.id, {
                  dataBinding: { source: v as any, query: "", parameters: {}, refreshInterval: 0 }
                })}
              >
                <SelectTrigger className="h-8">
                  <SelectValue />
                ...
                <SelectContent>
                  ...
                  <SelectItem value="workflow-output">Workflow Output</SelectItem>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={e => updateWidget(selectedWidget.id, {
                    dataBinding: { ...selectedWidget.dataBinding!, query: e.target.value }
                  })}
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                onChange={e => updateWidget(selectedWidget.id, {
                  dataBinding: { ...selectedWidget.dataBinding!, refreshInterval: parseInt(e.target.value) || 0 }
                })}
                className="h-8 text-sm"
      ...
      <Dialog open={isPublishDialogOpen} onOpenChange={setIsPublishDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Publishing will make this dashboard visible to users with the appropriate permissions.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-1.5">
