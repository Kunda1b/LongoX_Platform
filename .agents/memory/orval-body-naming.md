...
- For endpoints where the body is a `$ref` to a schema with a _different_ name from `<OperationId>Body` (e.g. `createDashboard` with `$ref: DashboardInput`, body schema becomes `CreateDashboardBody`): no conflict, proceed normally.
