...
router.get("/dashboard/summary", authorize("dashboards:read"), async (_req, res): Promise<void> => {
  ...
  const todayExecs = executions.filter((e: any) => new Date(e.startedAt) >= today);
  const executionsToday = todayExecs.length;
  const failedToday = todayExecs.filter((e: any) => e.status === "failed").length;
...
router.get("/dashboard/activity", authorize("dashboards:read"), async (req, res): Promise<void> => {
  ...
  res.json(
    executions.map((e: any) => ({
      ...
      status: e.status,
      startedAt: e.startedAt instanceof Date ? e.startedAt.toISOString() : new Date(e.startedAt).toISOString(),
