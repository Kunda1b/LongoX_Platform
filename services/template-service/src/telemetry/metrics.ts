...
// Counters
export const templateInstallsCounter = meter.createCounter("template.installs", {
  description: "Total template installations",
});

export const templatePublishesCounter = meter.createCounter("template.publishes", {
  description: "Total template publishes",
});

// Histograms
export const templateOperationDurationHistogram = meter.createHistogram("template.operation.duration", {
...
export function recordTemplateInstall(
  ...
  templateInstallsCounter.add(1, { template_id: templateId, tenant_id: tenantId });
}
