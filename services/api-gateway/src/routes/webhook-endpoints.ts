...
import { db, webhookEndpointsTable, workflowsTable } from "@autoflow/db";
...
router.get("/webhook-endpoints", async (req, res): Promise<void> => {
  ...
  const rows = workflowId
    ? await db.select().from(webhookEndpointsTable).where(eq(webhookEndpointsTable.workflowId, workflowId)).orderBy(webhookEndpointsTable.id)
    : await db.select().from(webhookEndpointsTable).orderBy(webhookEndpointsTable.id);
...
router.post("/webhook-endpoints", async (req, res): Promise<void> => {
  const { workflowId, name, description, allowedIps, headers } = req.body as {
    workflowId?: number; name?: string; description?: string; allowedIps?: string[]; headers?: Record<string, string>;
  };
  if (!workflowId || !name?.trim()) {
    res.status(400).json({ error: "workflowId and name are required" }); return;
  }
...
router.get("/webhook-endpoints/:id", async (req, res): Promise<void> => {
  ...
  const [row] = await db.select().from(webhookEndpointsTable).where(eq(webhookEndpointsTable.id, id));
...
router.patch("/webhook-endpoints/:id", async (req, res): Promise<void> => {
  const id = parseInt(req.params.id, 10);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid id" }); return; }
  const b = req.body as Partial<{ name: string; description: string; isActive: boolean; allowedIps: string[]; headers: Record<string, string> }>;
  ...
  const [row] = await db.update(webhookEndpointsTable).set(updates).where(eq(webhookEndpointsTable.id, id)).returning();
  if (!row) { res.status(404).json({ error: "Not found" }); return; }
  res.json(fmt(row));
});

router.post("/webhook-endpoints/:id/regenerate-secret", async (req, res): Promise<void> => {
  const id = parseInt(req.params.id, 10);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid id" }); return; }
  const [row] = await db.update(webhookEndpointsTable).set({ secret: generateSecret() }).where(eq(webhookEndpointsTable.id, id)).returning();
  if (!row) { res.status(404).json({ error: "Not found" }); return; }
  res.json(fmt(row));
});
