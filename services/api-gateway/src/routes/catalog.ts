...
function serializeConnector(row: any) {
  return {
    ...
    certificationLevel:
      (row as any).certificationLevel ?? row.trustLevel ?? null,
  ...
  return {
    ...
    lastEditedAt:
      row.lastEditedAt instanceof Date
        ...
        : new Date(row.lastEditedAt).toISOString(),
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    if (req.query.installed)
      where.isInstalled = String(req.query.installed) === "true";
...
router.get(
  "/apps/stats",
  authorize({ resource: "apps", action: "read" }),
  async (_req, res): Promise<void> => {
...
router.post(
  "/apps",
  authorize({ resource: "apps", action: "write" }),
  async (req, res): Promise<void> => {
...
router.delete(
  "/apps/:id",
  authorize({ resource: "apps", action: "delete" }),
  async (req, res): Promise<void> => {
    const id = String(req.params.id);
    if (!Number.isInteger(Number(id))) {
      res.status(400).json({ error: "Invalid app id" });
...
router.get(
  "/credentials",
  authorize({ resource: "credentials", action: "read" }),
  async (_req, res): Promise<void> => {
...
router.post(
  "/credentials",
  authorize({ resource: "credentials", action: "write" }),
  async (req, res): Promise<void> => {
    const body = req.body as {
      name?: string;
      connectorId?: string;
