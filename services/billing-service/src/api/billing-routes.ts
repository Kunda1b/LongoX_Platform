...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    try {
      const usage = await meteringService.getMonthlyUsage(
        tenantId,
        year,
        month,
      );
  ...
  async (req, res): Promise<void> => {
    ...
    try {
      ...
      if (!(account as any)?.planId) {
        res.json({
          plan: null,
          limits: await entitlementService.getPlan(tenantId),
        });
        return;
      }
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    try {
      const usageAgainstPlan =
        await entitlementService.getUsageAgainstPlan(tenantId);
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    if (!eventId || !eventType || quantity == null || !unit) {
      res
        .status(400)
        .json({ error: "eventId, eventType, quantity, and unit are required" });
      return;
    }
