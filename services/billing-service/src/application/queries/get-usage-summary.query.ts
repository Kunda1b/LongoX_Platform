...
export class GetUsageSummaryQuery {
  ...
  async execute(): Promise<UsageSummary> {
    const now = new Date();
    const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);

    const metrics = await this.repository.getMetrics(monthStart);
    const monthEvents = await this.repository.getEventQuantities(monthStart);
