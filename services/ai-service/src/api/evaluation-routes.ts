...
router.get(
  ...
  async (req, res): Promise<void> => {
    try {
      const dataset = await evaluationDatasetService.getDataset(
...
router.post(
  ...
  async (req, res): Promise<void> => {
    try {
      ...
      const inserted = await evaluationDatasetService.addEntries(
        String(req.params.id),
        entries,
      );
...
router.delete(
  ...
  async (req, res): Promise<void> => {
    try {
      const dataset = await evaluationDatasetService.deleteDataset(
        String(req.params.id),
      );
      if (!dataset) {
        res.status(404).json({ error: "Dataset not found" });
...
router.get(
  ...
  async (req, res): Promise<void> => {
    try {
      const results = await evaluationRunService.getRunResults(
        String(req.params.id),
      );
      res.json(results);
    } catch (error) {
  ...
  async (req, res): Promise<void> => {
    try {
      ...
      if (!promptId || !promptVersion) {
        res
          .status(400)
          .json({ error: "promptId and promptVersion are required" });
        return;
      }
      ...
      const gateResult = targetEnvironment
        ? await regressionGateService.checkPromotionGate(
            promptId,
            promptVersion,
            targetEnvironment,
          )
