...
router.patch(
  ...
  async (req, res): Promise<void> => {
    ...
    if (b.providerPreferences !== undefined)
      updates.providerPreferences = b.providerPreferences;
    if (b.modelAllowlist !== undefined)
      updates.modelAllowlist = b.modelAllowlist;
    if (b.modelDenylist !== undefined) updates.modelDenylist = b.modelDenylist;
    if (b.fallbackEnabled !== undefined)
      updates.fallbackEnabled = b.fallbackEnabled;
    ...
    const row = await prisma.aiRoutingPolicy
      .update({
        where: { id } as any,
        data: updates as any,
      })
