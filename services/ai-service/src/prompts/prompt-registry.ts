import { db, promptsTable, templateVersionsTable } from "@autoflow/db";
...
export class PromptRegistry {
  async getPrompt(id: number): Promise<PromptDefinition | null> {
    const [prompt] = await db.select().from(promptsTable).where(eq(promptsTable.id, id)).limit(1);
    ...
    return {
      ...
      variables: typeof prompt.variables === "string" ? JSON.parse(prompt.variables) : (prompt.variables as string[] ?? []),
      version: this.getVersionNumber(prompt.createdAt),
    };
  }

  async listPrompts(limit: number = 50): Promise<PromptDefinition[]> {
    const rows = await db.select().from(promptsTable).orderBy(promptsTable.id).limit(limit);
    return rows.map((p) => ({
      id: p.id,
      name: p.name,
      ...
      variables: typeof p.variables === "string" ? JSON.parse(p.variables) : (p.variables as string[] ?? []),
      version: this.getVersionNumber(p.createdAt),
    }));
  }

  async createPrompt(data: Omit<PromptDefinition, "id" | "version">): Promise<PromptDefinition> {
  ...
  async updatePrompt(id: number, data: Partial<Omit<PromptDefinition, "id" | "version">>): Promise<PromptDefinition | null> {
    const updates: Record<string, unknown> = {};
    if (data.name !== undefined) updates.name = data.name;
    if (data.description !== undefined) updates.description = data.description;
    if (data.systemPrompt !== undefined) updates.systemPrompt = data.systemPrompt;
    if (data.userTemplate !== undefined) updates.userTemplate = data.userTemplate;
  ...
  async renderPrompt(promptId: number, variables: Record<string, string>): Promise<{ systemPrompt: string; userPrompt: string }> {
    ...
    for (const [key, value] of Object.entries(variables)) {
      ...
      userTemplate = userTemplate.replace(new RegExp(`\\{\\{${key}\\}\\}`, "g"), value);
    }
