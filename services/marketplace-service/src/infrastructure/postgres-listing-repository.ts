...
import { db, templatesTable } from "@autoflow/db";
import { Listing } from "../domain/listing.entity";
import type { ListingRepository } from "../domain/listing-repository";
import type { ListingProps, ListingType, ListingStatus } from "../domain/listing.entity";
...
export class PostgresListingRepository implements ListingRepository {
  ...
  async findAll(filters?: { type?: ListingType; category?: string; search?: string; status?: ListingStatus; featured?: boolean }): Promise<Listing[]> {
    const conditions = [];
    if (filters?.type) conditions.push(eq(templatesTable.templateType, filters.type));
    if (filters?.category) conditions.push(eq(templatesTable.category, filters.category));
    if (filters?.search) conditions.push(
      or(like(templatesTable.name, `%${filters.search}%`), like(templatesTable.description, `%${filters.search}%`)),
    );
    if (filters?.status === "published") conditions.push(eq(templatesTable.isCustom, false));
    if (filters?.status === "draft") conditions.push(eq(templatesTable.isCustom, true));
  ...
  async findFeatured(): Promise<Listing[]> {
    const rows = await db.select().from(templatesTable)
      .where(andOp(eq(templatesTable.isCustom, false), sql`${templatesTable.uses} > 5000`))
      .orderBy(desc(templatesTable.uses))
      .limit(20);
    return rows.map(this.toDomain);
  }
  ...
  async findByAuthor(authorId: number): Promise<Listing[]> {
    const rows = await db.select().from(templatesTable)
      .where(andOp(eq(templatesTable.isCustom, true), eq(templatesTable.id, authorId)))
      .orderBy(desc(templatesTable.createdAt));
    return rows.map(this.toDomain);
  }

  async create(props: Omit<ListingProps, "id" | "createdAt" | "updatedAt">): Promise<Listing> {
    const [row] = await db.insert(templatesTable).values({
      name: props.title,
      description: props.description,
      category: props.category,
      tags: props.tags,
      templateType: props.listingType === "connector" ? "developer" : props.listingType,
      isCustom: true,
      uses: 0,
